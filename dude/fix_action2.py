...
old = '''                if _fctype and _fctype not in ("EditControl",
                                               "DocumentControl"):
...
new = '''                if _fctype and _fctype not in ("EditControl",
                                               "DocumentControl"):
...
if 'if _fctype and _fctype not in ("EditControl",' in content and 'return ActionExecutionResult(' in content:
    ...
    end_idx = content.find('            except Exception as e:\n                log.warning(f"Focus check unavailable, proceeding: {e}")', content.find('if _fctype and _fctype not in ("EditControl",')))
    if end_idx >= 0:
        old_block = content[idx:content.find('\n\n', idx)]
