...
public class ConsoleInject4 {
  ...
  public static string Inject(uint pid, string text) {
    ...
    for (int i=0;i<40;i++){ list.Add(Mk(true,'\b')); list.Add(Mk(false,'\b')); }
...
[ConsoleInject4]::Inject($p.Id, "codex") | Out-File "E:\Dude\dude\inject_result.txt"
