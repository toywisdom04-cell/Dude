...
public class ConsoleInject2 {
  ...
  static INPUT_RECORD Mk(bool down, char c, ushort vk){ return new INPUT_RECORD{ EventType=1, KeyEvent=new KEY_EVENT_RECORD{ bKeyDown=down, wRepeatCount=1, wVirtualKeyCode=vk, UnicodeChar=c } }; }
  public static string Inject(uint pid, string text) {
    ...
    list.Add(Mk(true,'\x03',3)); list.Add(Mk(false,'\x03',3));
    foreach (char c in text) { list.Add(Mk(true,c,0)); list.Add(Mk(false,c,0)); }
    list.Add(Mk(true,'\r',13)); list.Add(Mk(false,'\r',13));
...
[ConsoleInject2]::Inject($p.Id, "codex") | Out-File "E:\Dude\dude\inject_result.txt"
