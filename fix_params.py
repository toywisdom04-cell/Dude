...
old = """def _extract_parameters(self, pattern: ObservedPattern) -> list[ProcedureParameter]:
        ...
        if self._extractor:
            extraction = self._extractor.extract(pattern.goal, pattern.parameters)
...
new = """    def _extract_parameters(self, pattern: ObservedPattern) -> list[ProcedureParameter]:
        \"\"\"Extract parameter definitions from pattern contexts using ParameterExtractor.\"\"\"
        params = []
        param_names_seen = set()
